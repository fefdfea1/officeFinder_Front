...
export const ShowChat = (props: ChatProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      stompClient.current.disconnect(readMessage(props.roomId), queryClient.invalidateQueries("chatList"));
    };
  }, [props.roomId]);
  ...
  return (
    <div className="flex flex-col md:h-[485px] justify-between">
          ...
          {messages.map((message, index) =>
            !message.myMessage ? (
              <div key={index} className="chat chat-end">
                ...
                <time className="chat-footer text-sm pl-2 opacity-50 font-thin">
                  {ChatTimeFormating(message.createdAt)}
                </time>
              </div>
            ) : (
              <div key={index} className="chat chat-start pl-3">
                <div className="chat-header text-base pl-2">{`${props.userName}`}</div>
      ...
      <form
        className="bg-secondary w-full h-[46px] md:h-1/3 p-3 md:rounded-b-xl"
        onClick={e => {
          e.preventDefault();
        }}
      >
        <div className="relative h-full flex flex-row gap-2 md:flex-col items-end">
          ...
          />
          <button className="btn btn-primary btn-sm w-16 absolute bottom-[-5px] right-0" onClick={sendHandler}>
            send
          </button>
        </div>
      </form>
